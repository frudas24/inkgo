...
The project is stdlib-only, CGO-free, importable as `github.com/frudas24/inkgo`, and `v0.1.25` is the latest publicly tagged release. Development checkouts may contain additional `Unreleased` hardening.
...
`v0.1.19` makes the clipboard result synchronous: `SetClipboardSync` awaits the native write and returns whether it happened, because `SetClipboard`'s path only says which route exists. Both share one implementation.

`v0.1.18` adds the WSL clipboard bridge: `clip.exe` and a PowerShell stdin fallback join the Linux candidate list after the X11/Wayland tools, and `GetClipboardPath` reports `native` when only the Windows utilities exist. The native path remains refused across SSH.
...
The latest published tag is `v0.1.25`. New changes should remain under `Unreleased` until the next validated tag is created. Repository description/topics are GitHub metadata and are not part of source archives. See `RELEASE.md`.
