...
- invalidate on an epoch, not on `Node.subtreeGeomDirty`: a layout pass clears that flag whenever it visits a node, and the root is not measured at all in fullscreen mode, so a mutation followed by a fullscreen pass would clear the flag and leave the root's stale entry behind for the next natural-height pass, which then reports the pre-mutation height. `TestFlowMeasureCacheRootMeasuredOnlyOutsideFullscreen` drives exactly that sequence, and it is why the entry carries its own counter. `measureEpoch` is deliberately not `Node.generation`, which only bumps on an ancestor that was still clean and would miss a second mutation made before the first was visited;
- keep paint-only mutations out of the cache: `SetTextStyle` and scrolling change neither the epoch nor the key, so they keep the entries they cannot have changed (`TestFlowMeasureCachePaintOnlyMutationKeepsEntries`), and a resize with no mutation in between re-uses them as well (`TestFlowMeasureCacheResizeWithoutMutation`);
- bound the table, keep a miss cheap: every container memoises a fixed eight entries (`nodeFlowMeasureSlots`, a fixed array so a slot costs no slice header) and evicts oldest first, and a lookup serves only an exact `(epoch, availW, availH)` match, so exceeding the cap costs a recompute and never a wrong result. `TestFlowMeasureCacheKeySetIsBounded` records the keys a container really sees - 2 per container in the transcript shapes of the benchmarks, 3 in natural-height mode and 5 in a deeply nested auto-sized tree - and `TestFlowMeasureCacheAlternatesFlexAvails` alternates the two spaces the flex path uses and requires each to be served under its own key, then pushes five keys past the cap and requires every one of them, after the table has wrapped, to still return what a cold measurement returns;
- measured against the published tree, same host and same command (`go test ./internal/engine -run '^$' -bench BenchmarkTextUpdate -benchtime=200x`, AMD Ryzen 7 5800H, Go 1.26.2, v0.1.24 built from `git archive 5f34694`): the layout pass after a footer update drops from 59.9/551.4/2263.3 us to 4.5/5.2/11.1 us at n=20/200/800, 203x at n=800, and the pass stops allocating with the transcript - 6394 allocs and 206270 B at n=800 become 22 allocs and 708 B at every n. The fullscreen frame at n=800 drops from 3018.2 us to 682.1 us against a 664.3 us no-mutation floor (4.53x the floor becomes 1.03x), and the non-fullscreen frame at n=800 stays paint-bound at 27133.7 us against a 27530 us floor, which is what `BenchmarkTextUpdateFrameCost` keeps recording. The pass is not free: a second run of the same pair measured 9.3 us at n=800, and what remains there is dominated by `rebuildLayoutIndexes`, the full-tree walk every pass already performed (4.8-5.4 us at n=800 against 0.13 us at n=20), not by measuring;
- the adversarial case is a stale entry rather than a slow one: `TestFlowMeasureCacheStaleEntryAdversarial` mutates 22 ways (longer, shorter and multiline text; width, padding, gap, flex, position and display of a child; `Append`, `Remove`, `SetChildren`; two mutations with no pass between them; an auto-sized ancestor with a fixed sibling) and requires every measurement to agree with a twin that reaches the same state from scratch, while `TestFlowMeasureCacheServesRepeatedMeasurement` pins the saving itself (the repeated measurement walks one node where the first walks the subtree). `TestIncrementalLayoutProperty` gains a deep auto-sized tree shape whose 300 random mutations prune 213 passes and measure fewer nodes than the cold twin in 286 of them;
- no public API change and `go generate` leaves `api.go` untouched;

## v0.1.24 — incremental layout on content updates

- stop re-laying out a whole tree when one leaf's text changes: `Node.MarkDirty` walked the mutated node and its ancestors setting `layoutDirty`, and `ComputeLayout` reused the cached layout only while nothing in the tree was dirty, so a single spinner tick on a footer `Text` re-measured and re-wrapped every node and made a frame cost O(transcript). `MarkDirty` now marks the ancestor chain with a new `Node.subtreeGeomDirty`, and `layoutNode` leaves a subtree untouched when the rect its parent assigned and the rect it already holds agree and no node below it is geometrically dirty. Paint-only invalidation stays paint-only: `markPaintDirty` deliberately does not set the flag, so a style change still skips the layout pass entirely;
- keep the scroll bookkeeping of a skipped subtree correct: a pruned `ScrollBox` is never walked, so the `updateScrollState` call that `layoutNode` performs in place would be lost. `drainSkippedScroll` applies it once after the pass to exactly the scroll nodes the pass did not reach (each node records the pass stamp, so a scroll node is still updated exactly once per pass), and it skips the nodes the pass could never have updated - detached, under `DisplayNone`, or text content - so a pruned pass stays observationally identical to the full pass it stands in for;
- measured with `BenchmarkTextUpdateFrameCost*`, `BenchmarkTextUpdateFrameCostAltScreen` and `BenchmarkTextUpdateLayoutPassCost` (transcript-shaped tree: header plus a `ScrollBox` of n entries plus a footer, one frame per same-width footer update, AMD Ryzen 7 5800H, Go 1.26.2, `-benchtime=300x`): the layout pass after a footer update drops from 128.4/1236.7/5107.1 us to 52.1/526.3/2201.6 us at n=20/200/800, and a fullscreen frame from 863.7/2045.6/5877.7 us to 762.1/1222.6/2967.4 us - 2.0x at n=800. The prune is visible as a node count too: `TestIncrementalLayoutEquivalence` records a first pass over 206 nodes (n=200) and 806 nodes (n=800) against 5 nodes for the pass after the footer edit;
- the prune does not make a frame free, and the benchmarks record what it cannot remove: intrinsic measurement is still not memoised for containers (`measureNode` caches only text nodes, so a `Box`/`ScrollBox` re-walks its subtree through `measureFlowContent` whenever the flex pass measures it), and outside fullscreen the renderer sizes the screen to the laid-out content, where paint/diff is O(content) - `BenchmarkTextUpdateFrameCost/n=800` moves only from 32.6 ms to 29.6 ms per frame against the 27.1 ms no-mutation floor. `BenchmarkTextUpdateFrameCostNoMutation` is committed as that floor and `BenchmarkTextUpdateFrameCostAltScreen` as the ceiling the next change can raise;
- pin the equivalence rather than the speed: `TestIncrementalLayoutEquivalence` lays out a tree mutated in place and a twin that reaches the same state from scratch over 23 mutation cases (same-width and re-wrapping text, `RawANSI`, button state, `DisplayNone` round trip, append/remove, relative and absolute children, the four scroll drain paths, and a previously pruned subtree that has to repropagate) and requires identical rects, content rects, scroll state and `childrenLinearY`; `TestIncrementalLayoutProperty` drives 400 deterministic pseudo-random mutations, three viewport sizes included, through the public mutation API and compares every step against a full pass used as the oracle; `TestIncrementalLayoutScrollDrainInFullPass`, `TestIncrementalLayoutPaintOnlyDoesNotRelayout` and `TestIncrementalLayoutMutationProperty` cover the three ways the prune itself could rot - a drain that loses a scroll update, a paint-only change that starts walking the tree, and a mutation aimed at the child the previous pass pruned;
- no public API change and `go generate` leaves `api.go` untouched;
...
- add a printable-ASCII row producer: a line whose bytes are all 0x20-0x7e has no escapes to strip, no tab stops to expand and no zero-width control bytes, so `hardWrapLine` now wraps it by scanning bytes and slicing the source line instead of building a token per grapheme. `MeasureText` on a 1024-byte entry drops from about 480-507 us to about 4 us per call, and the same differential treatment applies: `TestWrapPlainASCIIMatchesTokenPath` compares the byte path against the token path over 8,400 randomized and targeted inputs (space runs at both boundaries, empty words, overlong words, widths at and below the word length) and requires byte-identical rows;
- make one row-producing helper serve both wrap and measure: `MeasureText` wrapped into a string, split it again and re-measured every row, re-deriving on the layout hot path exactly what the wrap pass had already computed. `WrapText`, `WrapTextLines` and `MeasureText` now share `wrappedRows`; `TestMeasureTextMatchesWrapDerivedRows` and `TestWrapTextLinesMatchesLegacyDerivation` pin the previous height/width and soft-wrap bitmap derivations over a corpus of ASCII, ANSI, tab, CRLF, CJK and emoji input;
- add `BenchmarkWrapPlainASCII`, `BenchmarkMeasureTextPlainASCII`, `BenchmarkMeasureTextTranscript`, `BenchmarkWrapStyledASCII` and `BenchmarkRendererResizeLongTranscript`, which are measurably slow on the previous tree (604 us, 559 us, 102.5 ms, 572 us and 314 ms/op respectively; 4.7 us, 5.0 us, 0.84 ms, 595 us and 58 ms/op after) so the plain-text fast path cannot be dropped silently, while the styled-line benchmark stays as the control for the token path;
