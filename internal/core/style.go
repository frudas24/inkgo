...
// ApplyDefaults resolves zero-value style fields to the library defaults.
func ApplyDefaults(s *Style) { s.defaults() }

// MarginEdges resolves the effective margin box.
func MarginEdges(s Style) Edges { return s.marginEdges() }

// PaddingEdges resolves the effective padding box.
func PaddingEdges(s Style) Edges { return s.paddingEdges() }

// BorderEdges resolves enabled border widths.
func BorderEdges(s Style) Edges { return s.borderEdges() }

// GapMain resolves the main-axis gap for dir.
func GapMain(s Style, dir FlexDirection) int { return s.gapMain(dir) }

// GapCross resolves the cross-axis gap for dir.
func GapCross(s Style, dir FlexDirection) int { return s.gapCross(dir) }
