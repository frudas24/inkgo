...
func copyScreenInto(dst, src *Screen) {
	if dst == nil || src == nil {
		return
	}
	...
	if cap(dst.SoftWrap) < len(src.SoftWrap) {
		dst.SoftWrap = make([]bool, len(src.SoftWrap))
	} else {
		dst.SoftWrap = dst.SoftWrap[:len(src.SoftWrap)]
	}
	copy(dst.SoftWrap, src.SoftWrap)
	if cap(dst.SoftWrapEnd) < len(src.SoftWrapEnd) {
		dst.SoftWrapEnd = make([]int, len(src.SoftWrapEnd))
	} else {
		dst.SoftWrapEnd = dst.SoftWrapEnd[:len(src.SoftWrapEnd)]
	}
	copy(dst.SoftWrapEnd, src.SoftWrapEnd)
}
